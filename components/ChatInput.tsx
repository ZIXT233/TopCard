...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const handleSend = useCallback(async () => {
    ...
    if (newSessionModelBlock) return;
    onAudioUnlock?.();
  ...
  }, [value, attachedImages, isStreaming, runBuiltinCommand, onSend, clearInput, onAudioUnlock, newSessionModelBlock]);
  ...
  const toolPermissionLabel = t("chat.toolPermission", { preset: toolPresetLabel });
      ...
      <div>
        ...
        <div style={{ position: "relative", minWidth: 0 }}>
          ...
          {isStreaming ? (
            ...
            <button
              onClick={handleSend}
              disabled={Boolean(newSessionModelBlock) || (!value.trim() && !attachedImages.length)}
              style={{
                flexShrink: 0,
                alignSelf: "flex-end",
                display: "flex", alignItems: "center", gap: 6,
            ...
            {!isStreaming && onToolPresetChange && (
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  disabled={isStreaming}
                  title={t("chat.changeToolPreset") + `: ${toolPermissionLabel}`}
                ...
                >
                  ...
                  {(!isMobile || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{toolPermissionLabel}</span>}
                </button>
                {toolDropdownOpen && (
                  <div style={{
                    position: "absolute",
