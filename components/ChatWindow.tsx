...
export function ChatWindow({ session, searchTarget, onSearchTargetHandled, initialScrollPosition, onScrollPositionChange, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd, onPromptSubmitted, onPromptAccepted, onPromptRejected, onAttentionNeeded, onSessionCreated, onSessionForked, modelsRefreshKey, onOpenModelSettings, chatInputRef, onBranchDataChange, onSystemPromptChange, onSystemToolsChange, onSystemInfoLoaderChange, onSessionStatsChange, onSessionStatsPanelOpen, onContextUsageChange, onOpenFile, onOpenSession, onAskInNewChat, quoteSelectionEnabled = false, initialPrompt, newSessionHeading, onInitialPromptConsumed, soundEnabled = true, onSoundToggle, playDoneSound = () => {}, unlockAudio }: Props) {
  const { t } = useI18n();
  ...
  const {
    ...
    agentRunning, bashRunning, pendingBash, modelNames, modelList, modelError, newSessionModelBlock, newSessionModelStatus, modelScopeWarnings, modelThinkingLevels, modelThinkingLevelMaps, toolPreset, thinkingLevel,
    ...
    retryModels, handleSend, handleAbort, handleFork, handleNavigate, handleModelChange,
    ...
    session, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd: wrappedOnAgentEnd, onPromptSubmitted, onPromptAccepted, onPromptRejected, onAttentionNeeded, onSessionCreated, onSessionForked,
  ...
  useEffect(() => {
    if (loading || error || newSessionModelStatus === "loading" || !initialPrompt || initialPromptSentRef.current) return;
    initialPromptSentRef.current = true;
    onInitialPromptConsumed?.();
    void handleSend(initialPrompt);
  }, [initialPrompt, loading, error, newSessionModelStatus, handleSend, onInitialPromptConsumed]);
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden">
        ...
        >
          <div style={{ minWidth: 0, padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
            <div ref={messageContentRef} onPointerUp={captureQuotedSelection} style={{ width: "100%", minWidth: 0 }}>
            {(() => {
              let lastUserIdx = -1;
      ...
      <div className="relative shrink-0">
        {isEmptyNew && (
          <div className="mx-auto mb-3 w-full" style={{ paddingLeft: 32, paddingRight: isMobile ? 32 : 68 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, fontFamily: "var(--font-mono)" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: isMobile ? 7 : 10, minWidth: 0, flex: 1, lineHeight: 1.4, overflow: "hidden" }}>
                ...
                <span style={{ fontSize: 22, color: "var(--text)", fontWeight: 700, flexShrink: 0, whiteSpace: "nowrap" }}>{newSessionHeading ?? "TopCard"}</span>
              </div>
            </div>
          </div>
        )}
