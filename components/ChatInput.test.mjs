...
test("renders the read-only tool preset as the active selection", () => {
  ...
  assert.match(html, /title="Change tool preset: Permissions: read-only"/);
  assert.match(html, />Permissions: read-only<\/span>/);
});
...
test("renders the empty tool preset as Chat only", () => {
  ...
  assert.match(html, /title="Change tool preset: Permissions: Chat only"/);
  assert.match(html, />Permissions: Chat only<\/span>/);
});
...
test("new-session model guard preserves text and attachments before any command or send", async () => {
  const source = ts.createSourceFile("ChatInput.tsx", readFileSync(new URL("./ChatInput.tsx", import.meta.url), "utf8"), ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  function findHandler(node) {
    if (ts.isVariableDeclaration(node) && node.name.getText(source) === "handleSend") return node.initializer.arguments[0];
    return ts.forEachChild(node, findHandler);
  }
  const script = new Script(ts.transpileModule(findHandler(source).getText(source), {
    compilerOptions: { target: ts.ScriptTarget.ES2020 },
  }).outputText);
  for (const block of ["Loading models", "No available models", "Model loading failed", null]) {
    const calls = [];
    const attachedImages = [{ data: "image-data", mimeType: "image/png" }];
    const send = script.runInNewContext({
      value: "keep my draft", attachedImages, newSessionModelBlock: block, isStreaming: false,
      onAudioUnlock: () => {}, runBuiltinCommand: async () => { calls.push("command"); return false; },
      clearInput: () => calls.push("clear"),
      onSend: (message, images) => { assert.equal(message, "keep my draft"); assert.equal(images, attachedImages); calls.push("send"); },
    });
    await send();
    assert.deepEqual(calls, block ? [] : ["command", "clear", "send"]);
  }
});
