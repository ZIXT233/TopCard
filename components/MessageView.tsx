...
import { TurnTagNotice, TurnTagRuleNotice } from "./TurnTagNotice";
import { hideTurnTags, TURN_TAG_RULES } from "@/lib/turn-tag-protocol";
...
export const MessageView = memo(function MessageView({ message, isStreaming, toolResults, modelNames, cwd, onOpenFile, onOpenSession, entryId, searchBlock, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent, showTimestamp, prevTimestamp, sessionId, writtenFiles }: Props) {
  ...
  if (message.role === "custom") {
    ...
    if ((message as CustomMessage).customType === TURN_TAG_RULES) return <TurnTagRuleNotice message={message as CustomMessage} />;
...
function AssistantMessageView({
  ...
  return (
    ...
    >
      ...
      {!isStreaming && message.stopReason !== "toolUse" && (textContent || message.turnTagRaw) && <TurnTagNotice message={message} />}
...
function TextBlock({ block, isStreaming, cwd, onOpenFile }: { block: TextContent; isStreaming?: boolean; cwd?: string; onOpenFile?: (filePath: string) => void }) {
  return <SafeMarkdownBody isStreaming={isStreaming} cwd={cwd} onOpenFile={onOpenFile}>{hideTurnTags(block.text)}</SafeMarkdownBody>;
}
