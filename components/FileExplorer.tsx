...
function TreeNode({
  ...
  useEffect(() => { if (open && !loaded) void loadChildren(); }, [open, loaded, loadChildren]);
  ...
  return (
    <div>
      <div
        data-upload-directory={node.isDir ? node.fullPath : getFileDirectory(node.fullPath)}
        ...
        style={{
          ...
          height: 24,
          cursor: "pointer",
          background: dropDirectory === node.fullPath ? "var(--bg-selected)" : hovered ? "var(--bg-hover)" : "transparent",
          borderRadius: 4,
          userSelect: "none",
        }}
      >
...
export const FileExplorer = forwardRef<FileExplorerHandle, Props>(function FileExplorer({
  ...
  const pickerDirectory = useRef(cwd);
  const uploadLock = useRef(false);
  const uploadContext = useRef({ cwd });
  if (uploadContext.current.cwd !== cwd) { uploadContext.current = { cwd }; uploadLock.current = false; }
  const [dropDirectory, setDropDirectory] = useState<string | null>(null);
  const [uploadMenu, setUploadMenu] = useState<{ directory: string; x: number; y: number } | null>(null);
  const explorerRoot = useRef<HTMLDivElement>(null);
  useEffect(() => {
    uploadContext.current = { cwd };
    return () => { uploadContext.current = { cwd: "" }; };
  }, [cwd]);
  useEffect(() => {
    if (!uploadMenu) return;
    const dismiss = () => setUploadMenu(null);
    window.addEventListener("pointerdown", dismiss);
    window.addEventListener("blur", dismiss);
    return () => { window.removeEventListener("pointerdown", dismiss); window.removeEventListener("blur", dismiss); };
  }, [uploadMenu]);
  ...
  const applyUploadResult = useCallback((data: UploadResponse, directory: string) => {
    const uploaded = data.uploaded ?? [];
    const skipped = data.skipped ?? [];
    const errors = data.errors ?? [];
    setUploadSummary({ directory, uploaded, skipped, errors });
    ...
    if (uploaded.length > 0) {
      ...
      setExpandedPaths(previous => {
        ...
        let path = directory;
        while (path !== cwd && path.startsWith(cwd.replace(/\/$/, "") + "/")) {
          next.add(path); path = getFileDirectory(path);
        }
        return next;
      });
  ...
  const performUpload = useCallback(async (
    ...
    try {
      const { status, data } = await uploadFiles(directory, files, strategy, progress => { if (uploadContext.current === context) setUploadProgress(progress); });
      if (uploadContext.current !== context) return;
      ...
      applyUploadResult(data, directory);
    } catch (uploadFailure) {
      if (uploadContext.current === context) setUploadError(uploadFailure instanceof Error ? uploadFailure.message : String(uploadFailure));
    } finally {
      if (uploadContext.current === context) { setUploadPhase("idle"); uploadLock.current = conflict; }
    }
  }, [applyUploadResult]);

  const prepareUpload = useCallback(async (files: File[], directory = cwd) => {
    if (files.length === 0 || uploadLock.current) return;
    const error = dropFilesError(files);
    if (error) { setUploadError(t(error)); return; }
    const context = uploadContext.current;
    uploadLock.current = true;
    let conflict = false;
    ...
    try {
      ...
      await performUpload(files, "error", directory);
      // performUpload owns lock state, including a conflict that appeared after the check.
      conflict = uploadLock.current;
    } catch (uploadFailure) {
      if (uploadContext.current === context) setUploadError(uploadFailure instanceof Error ? uploadFailure.message : String(uploadFailure));
    } finally {
      if (uploadContext.current === context) { setUploadPhase("idle"); uploadLock.current = conflict; }
  ...
  const handleUploadInput = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = "";
    void prepareUpload(files, pickerDirectory.current);
  }, [prepareUpload]);

  useImperativeHandle(ref, () => ({
    openUploadPicker() {
      if (!uploadLock.current) { pickerDirectory.current = cwd; uploadInputRef.current?.click(); }
    },
  }), [cwd]);
  ...
  const addUploadedFilesToChat = useCallback(() => {
    ...
    onAtMentions?.(
      uploadSummary.uploaded.map((name) => getRelativeFilePath(joinFilePath(uploadSummary.directory, name), cwd)),
    );
  }, [cwd, onAtMentions, uploadSummary]);

  return (
    <div ref={explorerRoot} className="file-explorer" style={{ minHeight: "100%" }}
      onDragOver={(event) => {
        if (!isFileDrag(event.dataTransfer)) return;
        event.preventDefault(); event.stopPropagation();
        event.dataTransfer.dropEffect = uploadLock.current ? "none" : "copy";
        if (!uploadLock.current) setDropDirectory((event.target as Element).closest<HTMLElement>("[data-upload-directory]")?.dataset.uploadDirectory ?? cwd);
      }}
      onDragLeave={(event) => { if (!(event.relatedTarget instanceof Node) || !event.currentTarget.contains(event.relatedTarget)) setDropDirectory(null); }}
      onDrop={(event) => {
        if (!isFileDrag(event.dataTransfer)) return;
        event.preventDefault(); event.stopPropagation(); setDropDirectory(null);
        const directory = (event.target as Element).closest<HTMLElement>("[data-upload-directory]")?.dataset.uploadDirectory ?? cwd;
        try { void prepareUpload(droppedFiles(event.dataTransfer), directory); }
        catch (error) { setUploadError(t(error instanceof Error ? error.message : String(error))); }
      }}
      onContextMenu={(event) => {
        event.preventDefault(); event.stopPropagation();
        const directory = (event.target as Element).closest<HTMLElement>("[data-upload-directory]")?.dataset.uploadDirectory ?? cwd;
        setUploadMenu({ directory, x: Math.min(event.clientX, window.innerWidth - 220), y: Math.min(event.clientY, window.innerHeight - 60) });
      }}
    >
      {dropDirectory && <div className="file-explorer-drop-overlay"><div className="file-explorer-drop-hint" role="status">{t("files.dropInto", { directory: getRelativeFilePath(dropDirectory, cwd) || getFileName(cwd) })}</div></div>}
      {uploadMenu && <div className="file-explorer-menu" role="menu" style={{ left: uploadMenu.x, top: uploadMenu.y }} onPointerDown={event => event.stopPropagation()} onKeyDown={event => { if (event.key === "Escape") setUploadMenu(null); }}>
        <button type="button" role="menuitem" disabled={uploadBusy || !!pendingConflict} autoFocus onClick={() => { pickerDirectory.current = uploadMenu.directory; uploadInputRef.current?.click(); setUploadMenu(null); }}>{t("files.uploadHere")}</button>
      </div>}
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, #f59e0b 55%, var(--border))", borderRadius: 4, background: "color-mix(in srgb, #f59e0b 9%, var(--bg-panel))" }}>
            <div style={{ fontSize: 11, color: "var(--text)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
              {t("files.dropInto", { directory: getRelativeFilePath(pendingConflict.directory, cwd) || getFileName(cwd) })}<br />
            ...
            <div style={{ display: "flex", gap: 5, marginTop: 7 }}>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "overwrite", pendingConflict.directory)} style={{ height: 22, padding: "0 7px", border: "1px solid #ef4444", borderRadius: 4, background: "transparent", color: "#ef4444", cursor: "pointer", fontSize: 10 }}>
                {t("files.replace")}
              </button>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "skip", pendingConflict.directory)} style={{ height: 22, padding: "0 7px", border: "1px solid var(--border)", borderRadius: 4, background: "var(--bg-panel)", color: "var(--text)", cursor: "pointer", fontSize: 10 }}>
                {t("files.skipExisting")}
              </button>
              <button type="button" onClick={() => { setPendingConflict(null); uploadLock.current = false; }} style={{ height: 22, padding: "0 7px", border: "none", borderRadius: 4, background: "transparent", color: "var(--text-muted)", cursor: "pointer", fontSize: 10 }}>
                {t("files.cancel")}
              </button>
            </div>
          </div>
