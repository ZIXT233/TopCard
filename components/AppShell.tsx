...
export function AppShell() {
  ...
  useEffect(() => {
    try {
      setQuoteSelectionEnabled(persistentStorage().getItem("pi-quote-selection-enabled") === "true");
    } catch {
  ...
  const handleQuoteSelectionChange = useCallback((enabled: boolean) => {
    ...
    try {
      persistentStorage().setItem("pi-quote-selection-enabled", String(enabled));
    } catch {
  ...
  // Preserve Escape handling for the running agent.
  ...
  const windowTitle = activeCwdName ? `${activeCwdName} - TopCard` : "TopCard";
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
          {showChat ? (
            <ChatWindow
              key={sessionKey}
              onOpenModelSettings={() => setSettingsSection("models")}
