...
function loadLastCustomCwd(): string {
  ...
  try {
    return persistentStorage().getItem(LAST_CUSTOM_CWD_STORAGE_KEY) ?? "";
  } catch {
    return "";
  }
}
...
function saveLastCustomCwd(cwd: string): void {
  if (typeof window === "undefined") return;
  try {
    persistentStorage().setItem(LAST_CUSTOM_CWD_STORAGE_KEY, cwd);
  } catch {
...
function loadUnreadSessionIds(): Set<string> {
  ...
  try {
    const raw = persistentStorage().getItem(UNREAD_SESSIONS_STORAGE_KEY);
...
function saveUnreadSessionIds(ids: Set<string>): void {
  if (typeof window === "undefined") return;
  try {
    if (ids.size === 0) persistentStorage().removeItem(UNREAD_SESSIONS_STORAGE_KEY);
    else persistentStorage().setItem(UNREAD_SESSIONS_STORAGE_KEY, JSON.stringify([...ids]));
  } catch {
...
function PiWebTitle() {
  ...
  const target = showVersion ? `${process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"}p${process.env.NEXT_PUBLIC_PI_VERSION ?? "0.0.0"}` : "TopCard";
