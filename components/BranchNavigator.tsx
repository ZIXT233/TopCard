...
export function BranchNavigator({ tree, activeLeafId, onLeafChange, inline, containerRef, open: openProp, onToggle, hasSession, compact, hideInlineButton, cardStyle }: Props) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    if (!open || !inline || !cardStyle) return;
    const outside = (event: Event) => {
      const target = event.target;
      if (target instanceof Node && !btnRef.current?.contains(target) && !dropdownRef.current?.contains(target)) close();
    };
    ...
    // The panel is portaled to body, so it does not inherit its card's inert
    // state. Dismiss it when the deck moves that card out of focus.
    const card = btnRef.current?.closest("article");
    const checkCard = () => {
      if (card?.matches('[inert], [aria-hidden="true"]')) close();
    };
    const observer = new MutationObserver(checkCard);
    if (card) observer.observe(card, { attributes: true, attributeFilter: ["inert", "aria-hidden"] });
    checkCard();
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const width = Math.min(cardStyle ? 280 : compact ? 340 : rect.width, window.innerWidth - 16);
      setDropdownPos({ top: rect.bottom + (cardStyle ? 8 : 0), left: Math.max(8, Math.min(cardStyle ? rect.right - width : rect.left, window.innerWidth - width - 8)), width });
    };
    update();
  ...
  }, [open, inline, containerRef, compact, cardStyle]);
  ...
  if (inline) {
    return (
      <div style={{ height: "100%", display: "flex", alignItems: "stretch" }}>
        <button
          ref={btnRef}
          className={cardStyle ? "cq-branch-trigger" : undefined}
          ...
          style={{
            ...
            gap: 6,
            height: "100%",
            padding: cardStyle ? 0 : "0 12px",
            width: cardStyle ? 32 : undefined,
            borderRadius: cardStyle ? 7 : undefined,
            background: open ? "var(--bg-selected)" : "none",
            border: "none",
            borderTop: !cardStyle && open ? "2px solid var(--accent)" : "2px solid transparent",
            borderRight: cardStyle ? "none" : "1px solid var(--border)",
            cursor: "pointer",
        ...
        {open && dropdownPos && createPortal(
          <div ref={dropdownRef} className={cardStyle ? "cq-branch-popover" : undefined} role="region" aria-label={t("i18n.branches")} style={{
            position: "fixed",
            top: dropdownPos.top,
            ...
            {cardStyle && <div className="cq-branch-heading">{t("i18n.branches")}</div>}
            {hasContent ? (
              ...
              <div style={{ padding: "10px 16px", fontSize: 12, color: "var(--text-muted)", fontStyle: cardStyle ? "normal" : "italic" }}>
                {noBranchReason}
              </div>
            )}
          </div>, document.body
