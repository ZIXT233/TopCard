...
async function startBackend() {
  ...
  const log = createWriteStream(join(logs, 'server.log'), { flags: 'w' });
  ...
  let failure;
  backend.once('error', error => { failure = new Error(`本地服务异常：${error.type}${error.location ? ` (${error.location})` : ''}`); });
  backend.once('exit', code => {
    backendExited = true;
    failure = new Error(`本地服务退出 (${code})，日志：${logs}`);
  ...
  if (!ready) throw new Error(`本地服务启动超时，日志：${logs}`);
  const createToken = () => {
...
async function requestSystemNotifications(verifyDelivery = false) {
  ...
  // Constructing/showing the first native notification invokes macOS authorization.
  ...
  notice.show();
  if (process.platform !== 'darwin' || !verifyDelivery) return 'requested';
...
function routeAppWindow(url, frameName) {
  ...
  if (cardId) showCard(cardId);
    ...
    if (attentionId && cardWindows.has(attentionId)) showCard(attentionId);
      ...
      showMain();
      // Preserve notification targets, including repeated clicks on the same target.
      // Root-only window requests still reuse the main window without a reload.
      if (attentionId || parsed.searchParams.has('session')) void mainWindow.loadURL(parsed.href);
    }
  }
  return true;
}
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(async () => {
    ...
    session.defaultSession.setPermissionRequestHandler((contents, permission, callback) => callback(trusted(contents.getURL()) && permission === 'notifications'));
    session.defaultSession.setPermissionCheckHandler((_contents, permission, requestingOrigin) => trusted(requestingOrigin) && permission === 'notifications');
