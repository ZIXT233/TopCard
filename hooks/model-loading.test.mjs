...
function setup(fetchImpl) {
  ...
  const context = {
    Error, DOMException,
    modelRequestKey: "project-models",
  ...
  for (const name of ["ModelAvailability", "ModelError", "ModelNames", "ModelScopeWarnings", "ModelThinkingLevels", "ModelThinkingLevelMaps", "ModelList", "NewSessionDefaultModel", "ThinkingLevel"]) {
...
test("model-load failures stay visible through bounded retries and clear on recovery", async () => {
  for (const [fetchImpl, expected] of [
    ...
    assert.deepEqual(state.writes.filter(([name]) => name === "ModelError"), Array.from({ length: 4 }, () => ["ModelError", expected]));
    assert.ok(state.writes.filter(([name]) => name === "ModelAvailability").every(([, value]) => value.status === "error"));
  }
...
test("model availability distinguishes empty lists, errors and plugin models", async () => {
  for (const [body, expected] of [
    [{ models: {}, modelList: [] }, "empty"],
    [{ models: {}, modelList: [], modelError: "Cannot load plugin" }, "error"],
    [{ models: {}, modelList: [{ provider: "cursor-plugin", id: "test", name: "Plugin model" }] }, "ready"],
  ]) {
    const state = setup(async () => Response.json(body));
    await state.run();
    const availability = state.writes.find(([name]) => name === "ModelAvailability")[1];
    assert.equal(availability.status, expected);
    assert.equal(availability.key, "project-models");
    assert.deepEqual(state.delays, []);
  }
});
