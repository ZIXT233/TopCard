...
export function useAgentSession(opts: UseAgentSessionOptions) {
  const {
    session, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd, onPromptSubmitted, onPromptAccepted, onPromptRejected, onAttentionNeeded, onSessionCreated, onSessionForked,
  ...
  const newSessionSubmissionRef = useRef(false);
  ...
  const restoreSubmission = useCallback((
    ...
    if (
      ...
      && !newSessionSubmissionRef.current
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false, signal?: AbortSignal) => {
    ...
    try {
      ...
      if (signal?.aborted || sessionIdRef.current !== sid) return null;
      ...
      try {
        const stateRes = await fetch(`/api/sessions/${encodeURIComponent(sid)}/state`, { signal });
        ...
        if (!signal?.aborted) console.error("Failed to load agent state:", e);
        return null;
      }
    } catch (e) {
      ...
      if (!signal?.aborted && showLoading && !messagesLoaded) setLoading(false);
    }
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[]) => {
    ...
    if (isNew && newSessionModelBlock) {
      restoreSubmission(message, images, composerDraftKey);
      addNotice({ type: "error", message: newSessionModelBlock });
      return;
    }
    ...
    if (isNew && !isSlashCommandPrompt) {
      newSessionSubmissionRef.current = true;
      onPromptSubmitted?.();
    }
    ...
    try {
      if (isNew && newSessionCwd) {
        ...
        sentSessionId = sid;
        if (newSessionDraftKey?.startsWith("card:")) {
          await sendAgentCommand(sid, { type: "apply_draft_prompt_sources", cardId: newSessionDraftKey.slice(5) });
        }
        if (selectedModel) {
        ...
        newSessionSubmissionRef.current = false;
      } else if (session) {
        ...
        if (!isSlashCommandPrompt) onPromptAccepted?.();
      } else {
      ...
      restoreSubmission(message, images, composerDraftKey);
      newSessionSubmissionRef.current = false;
      if (isNew && !isSlashCommandPrompt) onPromptRejected?.();
      optimisticUserMessageKeyRef.current = null;
  ...
  }, [isNew, newSessionModelBlock, newSessionCwd, newSessionDraftKey, newSessionModel, session, ensureNewSession, ensureEventsConnected, promoteNewSession, waitForPromptSettlement, addNotice, cancelEventStreamGrace, closeEvents, composerDraftKey, reconcileAgentState, restoreSubmission, onPromptSubmitted, onPromptAccepted, onPromptRejected]);
  ...
  const handleFork = useCallback(async (entryId: string) => {
    ...
    try {
      ...
      addNotice({ type: "error", message: `Failed to fork session: ${e instanceof Error ? e.message : String(e)}` });
    } finally {
  ...
  const loadModels = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!signal?.aborted && !(e instanceof DOMException && e.name === "AbortError")) {
        ...
        setModelAvailability({ key: modelRequestKey, status: "error" });
      }
      throw e;
    }
    ...
    setModelAvailability({ key: modelRequestKey, status: nextModelList.length ? "ready" : d.modelError ? "error" : "empty" });
  ...
  }, [isNew, newSessionCwd, session?.cwd, modelRequestKey]);
  ...
  // Load session on mount. Each effect owns its requests, including StrictMode replay.
  useEffect(() => {
    const controller = new AbortController();
    sessionHookMountedRef.current = true;
    if (session) {
      sessionIdRef.current = session.id;
      loadSession(session.id, true, true, controller.signal).then((agentState) => {
    ...
    return () => {
      ...
      if (abandonedDraftKey) {
        queueMicrotask(() => {
          if (!sessionHookMountedRef.current && !newSessionPromotedRef.current && !newSessionSubmissionRef.current) {
  ...
  return {
    ...
    agentRunning, modelNames, modelList, modelError, newSessionModelBlock, newSessionModelStatus, modelScopeWarnings, modelThinkingLevels, modelThinkingLevelMaps, newSessionModel, toolPreset, thinkingLevel,
    ...
    // Actions
    retryModels: () => { void loadModels().catch(() => {}); },
