...
test("fresh prompts leave the card deck before session and model startup", () => {
  const sendSource = source.slice(
    source.indexOf("  const handleSend = useCallback"),
    source.indexOf("  const executeBash = useCallback"),
  );
  ...
  assert.ok(sendSource.indexOf("onPromptSubmitted?.()") < sendSource.indexOf("await ensureNewSession()"));
  assert.match(sendSource, /restoreSubmission\(message, images, composerDraftKey\);[\s\S]*?onPromptRejected\?\.\(\)/);
  assert.match(loadEffectSource, /!newSessionPromotedRef\.current && !newSessionSubmissionRef\.current/);
});
