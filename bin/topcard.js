...
if (!fs.existsSync(nextDir)) {
  console.error("Build artifacts not found. Run `npm run build` before starting TopCard, or install a published release.");
  process.exit(1);
}
...
if (!loopbackHostnames.has(hostname)) {
  if (passwordEnabled) {
    console.warn(
      `Warning: TopCard is listening on ${hostname} with password authentication over HTTP. Use HTTPS or a trusted VPN to protect the password in transit.`,
    );
  } else {
    console.warn(
      `Warning: TopCard is listening on ${hostname} without authentication. Only use this on a trusted network.`,
    );
  }
}
