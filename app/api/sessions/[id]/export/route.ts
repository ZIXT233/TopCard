...
import { embeddedNodeExecutable, withEmbeddedNodeEnvironment } from "@/lib/node-runtime";
...
async function exportSession(filePath: string, outputPath: string): Promise<void> {
  ...
  if (cliPath) {
    await execFileAsync(embeddedNodeExecutable(), [cliPath, "--export", filePath, outputPath], {
      cwd: process.cwd(),
      timeout: 30_000,
      env: {
        ...withEmbeddedNodeEnvironment(),
        PI_OFFLINE: "1",
