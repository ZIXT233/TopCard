...
for (const method of ["select", "confirm", "input", "editor", "custom"]) {
  test(`Stop unwinds a command waiting on ${method} and closes its UI`, async (t) => {
    ...
    await waitFor(() => events.some((event) => event.method === method));
    ...
    await waitFor(() => events.some((event) => event.method === method && event.id !== request.id));
    const nextRequest = events.findLast((event) => event.method === method && event.id !== request.id);
