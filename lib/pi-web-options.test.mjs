...
const cliPath = fileURLToPath(new URL("../bin/topcard.js", import.meta.url));
const legacyCliPath = fileURLToPath(new URL("../bin/pi-web.js", import.meta.url));
...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /Usage: topcard/);
...
test("CLI writes help and parse errors before exiting", () => {
  ...
  assert.match(help.stdout, /Usage: topcard/);

  const legacyHelp = spawnSync(process.execPath, [legacyCliPath, "--help"], { encoding: "utf8" });
  assert.equal(legacyHelp.status, 0);
  assert.match(legacyHelp.stdout, /Usage: topcard/);
...
test("supports truthy TOPCARD_NO_OPEN and PI_WEB_NO_OPEN values", () => {
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { TOPCARD_NO_OPEN: value }).openBrowser, false);
    assert.equal(parseLaunchOptions([], { PI_WEB_NO_OPEN: value }).openBrowser, false);
  }
});

test("does not disable browser opening for false NO_OPEN values", () => {
  for (const value of ["0", "false", "off", ""]) {
    assert.equal(parseLaunchOptions([], { TOPCARD_NO_OPEN: value }).openBrowser, true);
...
test("supports TOPCARD_HOSTNAME without trusting the ambient system HOSTNAME", () => {
  assert.equal(
    parseLaunchOptions([], { HOSTNAME: "container-id" }).hostname,
    "127.0.0.1",
  );
  assert.equal(
    parseLaunchOptions([], { TOPCARD_HOSTNAME: "0.0.0.0" }).hostname,
    "0.0.0.0",
  );
  assert.equal(
  ...
  assert.equal(
    parseLaunchOptions([], { TOPCARD_HOSTNAME: "127.0.0.1", PI_WEB_HOSTNAME: "0.0.0.0" }).hostname,
    "127.0.0.1",
  );
});
