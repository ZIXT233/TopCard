import { hideTurnTags } from "./turn-tag-protocol.ts";
...
function normalizeAssistantToolCalls(
  ...
  const normalized = content.map((block) => {
    ...
    if (block.type === "text" && "text" in block && typeof block.text === "string") return { ...block, text: hideTurnTags(block.text) };
    return result ?? block;
  });
  const text = content.filter(block => block.type === "text").map(block => block.text).join("\n");
  const marker = text.search(/<turn_tags\s*>/);
  const turnTagRaw = marker >= 0 ? text.slice(marker) : (msg as AssistantMessage).turnTagRaw;
  return { ...msg, content: normalized, ...(turnTagRaw !== undefined ? { turnTagRaw } : {}) } as AgentMessage;
}
