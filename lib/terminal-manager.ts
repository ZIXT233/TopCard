import { readTerminalTranscript, saveTerminalTranscript } from "./terminal-transcript";
...
function scheduleCleanup(id: string, record: TerminalRecord): void {
  if (record.persistent || record.cleanupTimer || record.listeners.size || registry().get(id) !== record) return;
...
export function createTerminal(cwd: string, cols: number, rows: number, id: string = randomUUID(), launch?: TerminalLaunch): string {
  ...
  const pty = spawn(launch?.executable ?? shell, launch?.args ?? args, {
    name: "xterm-256color",
    cols: dimension(cols, 80),
    rows: dimension(rows, 24),
    cwd: cwd || homedir(),
    env: { ...shellEnvironment(), ...launch?.env },
  });
  const record: TerminalRecord = {
    pty,
    persistent: launch?.persistent,
    cwd,
    listeners: new Set(),
    backlog: "",
    offset: 0,
  ...
  pty.onData((data) => {
    ...
    launch?.onOutput?.(data);
    if (record.persistent && !record.saveTimer) {
      record.saveTimer = setTimeout(() => { record.saveTimer = undefined; saveTranscript(id, record); }, 1000);
...
export function hasTerminal(id: string): boolean {
  return registry().has(id) || !!readTerminalTranscript(id);
}

export function getTerminalCwd(id: string): string | undefined {
  return registry().get(id)?.cwd ?? readTerminalTranscript(id)?.cwd;
}
...
export function subscribeTerminal(
  ...
  if (!record) {
    const saved = readTerminalTranscript(id);
    return saved ? { output: { type: "output", data: saved.output, offset: saved.output.length, reset: true }, exited: true, exitCode: saved.exitCode, unsubscribe() {} } : null;
  }
...
export function getTerminalSnapshot(id: string) {
  const record = registry().get(id);
  return record ? { exited: record.exited, exitCode: record.exitCode, output: record.backlog } : null;
}

/** Stop an owned CLI, retaining its terminal buffer for history. */
export function stopTerminal(id: string): void {
  const record = registry().get(id);
  if (record && !record.exited) {
    record.pty.kill();
    const timer = setTimeout(() => { if (!record.exited) record.pty.kill("SIGKILL"); }, 2000);
    timer.unref?.();
  }
}

function saveTranscript(id: string, record: TerminalRecord) {
  if (record.persistent) saveTerminalTranscript(id, { cwd: record.cwd, output: record.backlog, exitCode: record.exitCode });
}
