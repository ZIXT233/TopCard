...
import { markQueueCardWorking, mergeQueueSnapshot, queueFallbackPollMs, queuePollIntervalMs, reuseIfEqual } from './card-queue-snapshot.ts';
...
test('idle visible boards poll slower than working boards', () => {
  const idle = { revision: 1, order: ['a'], cards: [{ id: 'a', phase: 'attention', harness: { state: 'attention' } }] };
  const busy = { revision: 1, order: [], cards: [{ id: 'b', phase: 'working', harness: { state: 'working' } }] };
  assert.equal(queuePollIntervalMs(null, true), 1200);
  assert.equal(queuePollIntervalMs(idle, true), 4000);
  assert.equal(queuePollIntervalMs(busy, true), 1200);
  assert.equal(queuePollIntervalMs(busy, false), 8000);
  assert.equal(queueFallbackPollMs(true, true), 8000);
  assert.equal(reuseIfEqual(idle.cards[0], { ...idle.cards[0] }), idle.cards[0]);
});
