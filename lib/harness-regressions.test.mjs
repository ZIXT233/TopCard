...
test('remote Codex preserves matching known identity and clears it after a different title prefix', async () => {
  const mocks = Object.fromEntries(['./windows-command','./bundled-pi','./local-environment','./shell-launch','./shell-probe','./hook-launch','./hook-osc','./codex','../terminal-transcript','./codex-title','./signals','./registry','../ssh-connection','../ssh-workspace'].map(k => [k, {}]));
...
test('Cursor ownership recognizes PowerShell EncodedCommand and stale remote helper paths', async () => {
  const hook = load('lib/harness/hook-launch.ts', { './inherited-config': {}, 'node:os': { homedir: () => '/tmp' }, '../ssh-workspace': { shellQuote: quote }, '../node-runtime': { embeddedNodeExecutable: () => '/usr/bin/node' } });
...
test('remote Codex launch passes hook configuration and token to its SSH PTY', async () => {
  ...
  const mocks = Object.fromEntries(['./windows-command','./bundled-pi','./local-environment','./shell-launch','./shell-probe','./codex','../terminal-transcript','./codex-title','./signals'].map(k => [k, {}]));
