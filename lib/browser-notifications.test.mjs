...
test("falls back to a page notification and wires its click handler", async () => {
  ...
  assert.deepEqual(notificationOptions, {
    title: "Session complete",
    options: { body: "Task finished.", icon: "/icons/topcard-192.png", silent: true },
  });
...
test("desktop notification clicks route the target instead of navigating the sending window", async () => {
  const { showBrowserNotification } = await loadSubject();
  const previous = globalThis.window;
  const targets = [];
  globalThis.window = { topcardDesktop: { openNotification: url => targets.push(url) } };
  const notification = { onclick: null, close() {} };
  try {
    await showBrowserNotification({ title: "Done", body: "", sessionUrl: "/?attention=card-b", onClick: () => assert.fail("must not navigate sender") }, {
      createWindowNotification: () => notification, getServiceWorkerRegistration: null,
    });
    notification.onclick();
    assert.deepEqual(targets, ["/?attention=card-b"]);
  } finally {
