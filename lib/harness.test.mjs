...
import { cursorHookStdout } from './harness/hook-contract.ts';
...
test('Codex plan and approval prompts raise attention after hooks already saw work', () => {
  const probe = codexAdapter.createProbe();
  assert.equal(probe.push('\x1b]9;Implement this plan?\x07'), 'attention');
  assert.equal(probe.consumeNeedsInput(), true);
  assert.equal(probe.consumeNeedsInput(), false);
  assert.equal(probe.push(title('[ ! ] Action Required')), 'attention');
  assert.equal(probe.consumeNeedsInput(), true);
  ...
  assert.equal(state.state, 'working');
  state = observeHook(state, { event: 'PermissionRequest', at: 30 });
...
test('Cursor permission hook replies ask and never allow', () => {
  assert.equal(cursorHookStdout('beforeSubmitPrompt'), '{"continue":true}');
  assert.equal(cursorHookStdout('beforeShellExecution'), '{"permission":"ask"}');
  assert.equal(cursorHookStdout('beforeMCPExecution'), '{"permission":"ask"}');
  assert.equal(cursorHookStdout('preToolUse'), '{}');
  assert.doesNotMatch(cursorHookStdout('beforeShellExecution'), /allow/);
});
...
test('hooks distinguish human input from tool completion and ignore child stops', () => {
  ...
  assert.equal(hookState({event:'beforeShellExecution'}),'attention');
  assert.equal(hookState({event:'beforeMCPExecution'}),'attention');
  assert.equal(hookState({event:'preToolUse'}),'working');
  assert.equal(hookState({event:'preToolUse',tool:'AskUserQuestion'}),'attention');
...
test('Cursor permission hooks raise attention after the card is bound', () => {
  let state=observeHook({state:'starting',at:0},{kind:'cursor',event:'sessionStart',at:1,sessionId:'bound'});
  state=observeHook(state,{kind:'cursor',event:'beforeSubmitPrompt',at:2,sessionId:'bound'});
  assert.equal(state.state,'working');
  state=observeHook(state,{kind:'cursor',event:'beforeShellExecution',at:3,sessionId:'bound'});
  assert.equal(state.state,'attention');
  state=observeHook(state,{kind:'cursor',event:'postToolUse',at:4,sessionId:'bound'});
  assert.equal(state.state,'working');
});
