...
import { mkdir, mkdtemp, readFile, rmdir, unlink, rm } from "node:fs/promises";
...
test('started sessions reject direct prompt edits and preserve legacy overrides', async () => {
  const entries = [{type:'custom',customType:'topcard:system-prompt',data:{prompt:'Legacy prompt'}}];
  const inner = { sessionManager: { getEntries: () => entries }, agent: { state: { systemPrompt: 'Default' } }, isStreaming:false, isCompacting:false, isBashRunning:false, dispose() {} };
  const wrapper = new AgentSessionWrapper(inner);
  assert.equal(inner.agent.state.systemPrompt,'Legacy prompt');
  await assert.rejects(wrapper.send({type:'set_system_prompt',prompt:'Changed'}));
  await assert.rejects(wrapper.send({type:'set_system_prompt',prompt:null}));
  assert.equal(inner.agent.state.systemPrompt,'Legacy prompt');
  wrapper.destroy();
});

test('draft source choices persist before an assistant exists and lock after a user message', async () => {
  const root=await mkdtemp(join(tmpdir(),'prompt-sources-'));
  const previous=process.env.TOPCARD_QUEUE_FILE;
  process.env.TOPCARD_QUEUE_FILE=join(root,'queue.json');
  const { withCardQueue }=await jiti.import('./card-queue-store.ts');
  const manager=SessionManager.create(root,root);
  const config={basePrompt:'Custom base',excludedContextFiles:[],excludedSkills:[]};
  const inner={sessionId:manager.getSessionId(),sessionManager:manager,agent:{state:{systemPrompt:'Default'}},extensionRunner:{},isStreaming:false,isCompacting:false,isBashRunning:false,dispose(){}};
  const wrapper=new AgentSessionWrapper(inner);
  try {
    await withCardQueue(state=>{state.cards=[{id:'draft',cwd:root,session:null,promptSources:config}];});
    assert.equal((await wrapper.send({type:'apply_draft_prompt_sources',cardId:'draft'})).changed,true);
    const restored=SessionManager.open(manager.getSessionFile());
    assert.deepEqual(restored.getEntries().find(e=>e.type==='custom'&&e.customType==='topcard:prompt-sources').data,config);
    restored.appendMessage({role:'user',content:'First request',timestamp:Date.now()});
    const next=new AgentSessionWrapper({...inner,sessionManager:restored});
    await withCardQueue(state=>{state.cards[0].promptSources={...config,basePrompt:'Changed'};});
    await assert.rejects(next.send({type:'apply_draft_prompt_sources',cardId:'draft'}));
    next.destroy();
  } finally {
    wrapper.destroy();
    if(previous===undefined)delete process.env.TOPCARD_QUEUE_FILE;else process.env.TOPCARD_QUEUE_FILE=previous;
