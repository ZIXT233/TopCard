import { basePartOfPrompt } from "./base-prompt-preview";
import { DEFAULT_PROMPT_SOURCES, PROMPT_SOURCES_ENTRY, promptSourceOverrides, validatePromptSources } from "./prompt-sources";
import { withCardQueue } from "./card-queue-store";
import { beginQueueTurn, injectTurnTagRules, recordQueueTurn } from "./turn-tag-evaluation";
import { TURN_TAG_RULES } from "./turn-tag-protocol";
import { validateTags } from "./turn-priority";
import type { ThinkingLevel } from "@earendil-works/pi-agent-core";
import { createAgentSessionFromServices, createAgentSessionServices, getAgentDir, initTheme, SessionManager, SettingsManager, Theme } from "@earendil-works/pi-coding-agent";
import { createSshWorkspaceExtension, loadSshWorkspace } from "./ssh-workspace";
...
export class AgentSessionWrapper {
  ...
  private sessionSystemPrompt: string | undefined;
  ...
  constructor(
    ...
    const promptEntry = inner.sessionManager?.getEntries?.().findLast(entry => entry.type === "custom" && entry.customType === "topcard:system-prompt");
    if (promptEntry?.type === "custom" && typeof (promptEntry.data as { prompt?: unknown })?.prompt === "string") this.sessionSystemPrompt = (promptEntry.data as { prompt: string }).prompt;
  ...
  needsHumanInput(): boolean {
    return this._alive && (this.pendingUiResponses.size > 0 || this.pendingUiRequests.size > 0);
  }
  ...
  start(): void {
    this.unsubscribe = this.inner.subscribe((event: AgentEvent) => {
      if (event.type === "agent_start") {
        ...
        void beginQueueTurn(this.sessionId).catch((error) => console.error("[card-queue] turn start:", error));
      }
  ...
  evaluateWaitingTurn(): void {
    if (this.isRunning()) return;
    const entries = this.inner.sessionManager.buildContextEntries();
    const final = entries.findLast(entry => entry.type === "message");
    if (!final || final.type !== "message" || final.message.role !== "assistant") return;
    if (final.message.stopReason !== "stop") return;
    ...
    void recordQueueTurn(this.sessionId, final.id, text, definitions)
      .catch(error => console.error("[card-queue] tag metadata:", error));
  }
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          shutdownHandler: () => this.emit({
            ...
            method: "notify",
            notifyType: "warning",
            message: "Extension requested shutdown, but shutdown is not supported in TopCard.",
  ...
  private applyExactSystemPrompt(): void {
    ...
    const prompt = this.sessionSystemPrompt ?? this.exactSystemPrompt?.();
    if (prompt !== undefined) this.inner.agent.state.systemPrompt = prompt;
  }

  private installExactSystemPromptContinuation(): void {
    if (!this.inner.agent) return;
    ...
    this.inner.agent.prepareNextTurnWithContext = async (turn, signal) => {
      ...
      const prompt = this.sessionSystemPrompt ?? this.exactSystemPrompt?.();
      if (prompt === undefined) return prepared;
      return {
        ...prepared,
        context: {
    ...
    try {
      ...
      case "prompt": {
        ...
        try {
          ...
          await injectTurnTagRules(this.inner, streamingBehavior ?? "nextTurn");
      ...
      case "set_system_prompt":
        throw new Error("完整系统提示词不可直接修改，请在新会话中配置提示词来源");
      case "apply_draft_prompt_sources": {
        const release = await this.acquirePromptAdmission();
        try {
          if (this.isRunning()) throw new Error("会话运行中不可修改提示词");
          const config = await withCardQueue(state => {
            const card = state.cards.find(item => item.id === command.cardId && !item.session && item.cwd === this.inner.sessionManager.getCwd());
            if (!card) return null;
            return card.promptSources ?? DEFAULT_PROMPT_SOURCES;
          });
          if (!config) return { changed: false };
          const previous = this.inner.sessionManager.getEntries().findLast(entry => entry.type === "custom" && entry.customType === PROMPT_SOURCES_ENTRY);
          if (JSON.stringify(previous?.type === "custom" ? previous.data : DEFAULT_PROMPT_SOURCES) === JSON.stringify(config)) return { changed: false };
          if (this.inner.sessionManager.getEntries().some(entry => entry.type === "message" && entry.message.role === "user")) throw new Error("会话开始后提示词只读");
          this.inner.sessionManager.appendCustomEntry(PROMPT_SOURCES_ENTRY, config);
          // Pi defers disk persistence until the first assistant reply. Flush
          // this pre-message configuration before rebuilding the resource loader.
      ...
      case "get_prompt_sources": {
        const entry = this.inner.sessionManager.getEntries().findLast(item => item.type === "custom" && item.customType === PROMPT_SOURCES_ENTRY);
        return {
          config: entry?.type === "custom" ? entry.data : DEFAULT_PROMPT_SOURCES,
          effectivePrompt: this.inner.agent.state?.systemPrompt ?? "",
          basePrompt: basePartOfPrompt(this.inner.agent.state?.systemPrompt ?? ""),
          contextFiles: this.inner.resourceLoader.getAgentsFiles().agentsFiles,
          skills: this.inner.resourceLoader.getSkills().skills.map(skill => ({ name: skill.name, description: skill.description, path: skill.filePath, disabled: skill.disableModelInvocation })),
          legacyOverride: this.sessionSystemPrompt !== undefined,
        };
      }
  ...
  private createExtensionUiContext(): ExtensionUiContextLike {
    return {
      ...
      getAllThemes: () => [],
      getTheme: () => undefined,
      setTheme: () => ({ success: false, error: "Theme switching is not supported in the TopCard extension UI yet" }),
...
export async function startRpcSession(
  ...
  const sourceEntry = sessionManager.getEntries().findLast(entry => entry.type === "custom" && entry.customType === PROMPT_SOURCES_ENTRY);
  const sourceConfig = sourceEntry?.type === "custom" ? validatePromptSources(sourceEntry.data) : DEFAULT_PROMPT_SOURCES;
  ...
  const starting = (async () => {
    const sshWorkspace = await loadSshWorkspace(sessionCwd);
    ...
    const services = await createAgentSessionServices({
      ...
      resourceLoaderOptions: { ...(subagentResources
        ...
        : {
            extensionFactories: [
              ...(sshWorkspace ? [createSshWorkspaceExtension(sessionCwd, sshWorkspace)] : [createProjectCommandBashExtension({
                cwd: sessionCwd,
        ...
        ...(!subagentResources ? promptSourceOverrides(sourceConfig) : {}),
      },
    ...
    const exactSystemPrompt = chatOnly
      ? subagentResources
        ...
        : () => [sourceConfig.basePrompt, contextFilesSystemPrompt(inner.resourceLoader.getAgentsFiles().agentsFiles)].filter(Boolean).join("\n\n")
      : undefined;
    const wrapper = new AgentSessionWrapper(inner, {
      exactSystemPrompt,
      chatOnly,
