...
test("opens TopCard settings once for navigation state saved before scope grouping", () => {
  const storage = createStorage({
    "pi-web:settings-navigation": JSON.stringify({ section: "models", selections: { models: "saved-model" } }),
  });

  assert.equal(getLastSettingsSection("/project", storage), "general");
  setLastSettingsSection("models", storage);
  assert.equal(getLastSettingsSection("/project", storage), "models");
});
