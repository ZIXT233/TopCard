...
const transcript = await jiti.import("./terminal-transcript.ts");
...
test("native module load failures are deferred until creation and include repair instructions", async () => {
  ...
  runInNewContext(outputText, { exports, process, require(id) {
    ...
    if (id === "./terminal-transcript") return transcript;
...
test("owned CLI terminals survive view disconnects and retain output after explicit stop", async (t) => {
  const { randomUUID } = await import("node:crypto");
  const { rmSync } = await import("node:fs");
  const { join } = await import("node:path");
  const { stopTerminal } = await jiti.import("./terminal-manager.ts");
  const id = randomUUID().replaceAll("-", "");
  createTerminal(process.cwd(), 80, 24, id, { executable: process.execPath, args: ["-e", 'console.log("owned-terminal-ready"); setInterval(()=>{},1000)'], persistent: true });
  t.after(() => { killTerminal(id); rmSync(join(process.cwd(), ".topcard", "terminal-transcripts", `${id}.json`), {force:true}); });
  const record = globalThis.__piWebTerminals.get(id);
  const connection = subscribeTerminal(id, () => {});
  connection.unsubscribe();
  assert.equal(record.cleanupTimer, null);
  const until = async predicate => { const start=Date.now(); while(!predicate()){assert.ok(Date.now()-start<4000);await new Promise(resolve=>setTimeout(resolve,20));} };
  await until(()=>record.backlog.includes("owned-terminal-ready"));
  stopTerminal(id);
  await until(()=>record.exited);
  assert.ok(subscribeTerminal(id,()=>{}).output.data.includes("owned-terminal-ready"));
  killTerminal(id);
  const restored = subscribeTerminal(id,()=>{});
  assert.equal(restored.exited,true);
  assert.ok(restored.output.data.includes("owned-terminal-ready"));
});
